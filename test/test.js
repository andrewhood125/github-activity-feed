...
describe('GitHub Activity Feed', function() {
  ...
  describe('#author()', function() {
    it('should return andrewhood125', function() {
      assert.equal("andrewhood125", gh.author(CreateEvent_tag));
    })
  });

  describe('#author_link()', function() {
    it('should return link to username', function() {
      assert.equal("<a href='https://github.com/andrewhood125'>andrewhood125</a>", gh.author_link(CreateEvent_tag));
    })
  });

  describe('#build_icon()', function() {
    it('should return html for octicon', function() {
      assert.equal("<span class='mega-octicon octicon-git-commit'></span>", gh.build_icon('mega-octicon octicon-git-commit'));
    });
  });

  describe('#convert_api_url()', function() {
    it('should return a github url with no subdomain', function() {
      assert.equal('https://github.com/', gh.convert_api_url('https://api.github.com/'));
    });
  });

  describe('#forkee_link()', function() {
    it('should return link to fork', function() {
      assert.equal("<a href='https://github.com/andrewhood125/common-critic'>andrewhood125/common-critic</a>", gh.forkee_link(ForkEvent.payload.forkee));
    });
  });

  describe('#gh_event()', function() {
    it('should return a single event object', function() {
      var e = gh.gh_event('commit', 'star', '5 days', '12pm');
  ...
  describe('#github_url()', function() {
    it('should return a github url for a resource', function() {
      assert.equal('https://github.com/andrewhood125', gh.github_url('andrewhood125'));
    });
  });

  describe('#link()', function() {
    it('should return html link for url and name', function() {
      assert.equal("<a href='andrew-hood.com'>Andrew Hood</a>", gh.link('andrew-hood.com', 'Andrew Hood'));
    });
  });

  describe('#pull_request_closed()', function() {
    it('should return true if pull request was closed', function() {
      assert.equal(true, gh.pull_request_closed(PullRequestEvent.payload));
    });
  });
  ...
  describe('#pull_request_link()', function() {
    it('should return an html link to the pull request', function() {
      assert.equal("<a href='https://github.com/andrewhood125/github-activity-feed/pull/6'>Add a Gitter chat badge to README.md</a>",
        gh.pull_request_link(PullRequestEvent.payload.pull_request));
    });
  });

  describe('#ref()', function() {
    it('should return ref', function() {
      assert.equal("0.1.4", gh.ref(CreateEvent_tag.payload));
    })
  });

  describe('#ref_link()', function() {
    it('should return link to ref', function() {
      assert.equal("<a href='https://github.com/andrewhood125/github-activity-feed/tree/0.1.4'>0.1.4</a>", gh.ref_link(CreateEvent_tag));
    });
  });

  describe('#ref_type()', function() {
    it('should return ref_type for the event', function() {
      assert.equal('tag', gh.ref_type(CreateEvent_tag.payload));
    });
  });

  describe('#remove_api_url()', function() {
    it('should strip the api subdomain from a github url', function() {
      assert.equal('andrewhood125', gh.remove_api_url('https://api.github.com/andrewhood125'));
    });
  });

  describe('#repository()', function() {
    it('should return the name of the repository', function() {
      assert.equal('andrewhood125/github-activity-feed', gh.repository(CreateEvent_tag.repo));
    });
  });

  describe('#repository_link()', function() {
    it('should return a link to the repository', function() {
      assert.equal("<a href='https://github.com/andrewhood125/github-activity-feed'>andrewhood125/github-activity-feed</a>", gh.repository_link(CreateEvent_tag));
    });
  });

  describe('#truncate()', function() {
    it('should truncate strings longer than 50 chars', function() {
      assert.equal('Short text', gh.truncate('Short text'));
      assert.equal('12345678910111213141516171819202122232425262728293...', gh.truncate('123456789101112131415161718192021222324252627282930'));
    });
  });
